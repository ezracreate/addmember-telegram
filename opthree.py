...
def directadder():
    ...
    groupset = config['Ezra']['fromgroup']
    grouplink = config['Ezra']['ToGroup']
    stopnum = config['Ezra']['EnterStop']
    stacno = config['Ezra']['StartingAccount']
    endacno = config['Ezra']['EndAccount']
...
def scraper():
    config = configparser.ConfigParser()
    config.read("config.ini")
    my_group = config['Ezra']['ToGroup']
    TARGET_group = config['Ezra']['fromgroup']
    api_hash = config["Ezra"]['api_hash']
    api_id = config["Ezra"]['api_id']
...
def fullscraper():
    ...
    config = configparser.ConfigParser()
    config.read("config.ini")
    link1 = (config['Ezra']['FromGroup']).strip()
    links= link1.split(',')
    API_ID = (config['Ezra']['api_id']).strip()
    HashID = (config['Ezra']['api_hash']).strip()
    my_group = config['Ezra']['ToGroup']
...
def adderrr():
    ...
    async def mainn():
        ...
        if pphone:
           ...
           print(f"There is no number on {Ezra_devinput} row'")
    asyncio.run(mainn())                  
